...
import { DAI, UNI, ZAI } from '../constants/tokens'
...
export const getTotalNet = async (dao) => {
  const daoContract = new web3.eth.Contract(daoAbi, dao)
  return daoContract.methods.totalNet().call()
}
...
export const getCost = async (amount) => {
  ...
  const [inputAmount, _] = await exchange.methods
    .getAmountsIn(new BigNumber(amount).toFixed(), [DAI.addr, ZAI.addr])
    .call()
  return inputAmount
}
...
export const getProceeds = async (amount) => {
  ...
  const [_, outputAmount] = await exchange.methods
    .getAmountsOut(new BigNumber(amount).toFixed(), [ZAI.addr, DAI.addr])
    .call()
  return outputAmount
}
...
export const getPoolFluidUntil = async (pool, account) => {
  ...
  const fromBlock = blockNumber - (POOL_EXIT_LOCKUP_EPOCHS + 1) * 540 // 30 minutes per epoch * 18 blocks per minute
  ...
  const endEpoch = events.reduce((endEpoch, event) => {
    if (endEpoch > parseInt(event.start, 10)) {
      return endEpoch
    } else {
      return parseInt(event.start, 10)
    }
  }, 0)

  return (endEpoch + POOL_EXIT_LOCKUP_EPOCHS).toString()
}

export const getLastPriceEvent = async (dao) => {
  const events = await getAllRegulations(dao)
